...
/**
 * Create flow — Step 1: Collection Details.
 * Identity, Trading rules, Revenue (royalty + mint funds split).
 */

import type { ShareAddressRow } from './create-types'
import { sanitizeSymbol, ROYALTY_SPLIT_MAX } from './create-types'
// Default import can be undefined with some bundlers after wallet connect; fallback avoids "Element type is invalid"
import FreezeUntilDatePickerModule from './FreezeUntilDatePicker'

function FreezeUntilDatePickerFallback({
  id,
  value,
  onChange,
  min,
...
const FreezeUntilDatePicker =
  (typeof FreezeUntilDatePickerModule === 'function'
    ? FreezeUntilDatePickerModule
    : (FreezeUntilDatePickerModule as { default?: typeof FreezeUntilDatePickerFallback })?.default) ??
  FreezeUntilDatePickerFallback
...
export default function CreateStep1Details(props: CreateStep1DetailsProps) {
  const {
    ...
    setSectionTradingExpanded,
    setSectionRevenueExpanded,
    isRoyaltySplitValid,
    fundReceiverError,
    step1Errors,
    collectionName,
    ...
    addRoyaltyConfig,
    removeRoyaltyConfig,
    distributeRoyaltyEvenly,
    autoFillRoyaltyRemainder,
    fundReceivers,
    updateFundReceiver,
    addFundReceiver,
    removeFundReceiver,
    distributeFundReceiversEvenly,
    autoFillFundReceiversRemainder,
    royaltySplitError,
    royaltyTotal,
    fundReceiverTotal,
  } = props
        ...
        <div className="nft-create-details-form">
          <div className="nft-create-step1-section nft-create-step1-section-identity">
            <div className="nft-create-step1-section-head">
              ...
              {identityValid && (
                <span className="nft-create-step1-badge" aria-label="Section complete">✓ Collection Identity Ready</span>
              )}
            </div>
            <div className="nft-create-details-fields-row nft-create-details-fields-wrap">
              <div className="nft-create-details-field">
                <label className="nft-create-details-label" htmlFor="create-collection-name">
                  Collection Name
                  <span className="nft-create-tooltip-wrap nft-create-tooltip-details" title="The name of your collection eg. 'My NFTs', will be displayed as the title of your collection.">
              ...
              <div className="nft-create-details-field">
                <label className="nft-create-details-label" htmlFor="create-symbol">
                  Symbol
                  <span className="nft-create-tooltip-wrap nft-create-tooltip-details" title="This is the ticker or symbol which will be used to represent your collection on the blockchain eg. 'MNFT'.">
                    <span className="nft-create-tooltip-icon" aria-hidden>i</span>
              ...
              <div className="nft-create-details-field nft-create-details-field-full">
                <label className="nft-create-details-label" htmlFor="create-collection-description">
                  Collection Description
                  <span className="nft-create-tooltip-wrap nft-create-tooltip-details" title="The description of your collection.">
                    <span className="nft-create-tooltip-icon" aria-hidden>i</span>
                    <span className="nft-create-tooltip-text">The description of your collection.</span>
                  </span>
                </label>
                <input
                ...
                <div className="nft-create-details-field">
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                      onChange={(e) => onFileSelect(e, 'collection')}
                      disabled={collectionImageUploading}
                    />
                    {collectionImageUploading ? (
                      <div className="nft-create-upload-loading">Uploading…</div>
                    ) : collectionImage ? (
                      <div className="nft-create-preview-card nft-create-step1-collection-preview">
                        <img src={collectionImage} alt="Collection" />
                      </div>
                    ) : (
                      <>
                        <div className="nft-create-upload-text">Drop or select image</div>
                        <div className="nft-create-upload-hint">PNG / JPG — square</div>
                      </>
                    )}
                  </label>
                  {collectionImageError && (
                    <span className="nft-create-inline-error" role="alert">{collectionImageError}</span>
                  )}
                </div>
                <div className="nft-create-details-field nft-create-step1-banner-field">
                  ...
                  >
                    ...
                    {bannerImageUploading ? (
                      ...
                      <div className="nft-create-preview-banner">
                        <img src={bannerImage} alt="Banner" />
                      </div>
                    ) : (
                      <>
                        <div className="nft-create-upload-text">Wide banner</div>
                        <div className="nft-create-upload-hint">PNG / JPG — 16:6</div>
                      </>
                    )}
                  </label>
                  {bannerImageError && (
                    <span className="nft-create-inline-error" role="alert">{bannerImageError}</span>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="nft-create-step1-collapse-body">
                      ...
                      <label key={opt.value} className="nft-create-radio-wrap">
                        <span className="nft-create-radio-input-wrap">
                          <input
                            ...
                            value={opt.value}
                            checked={metadataStandard === opt.value}
                            onChange={() => setMetadataStandard(opt.value)}
                          />
                        </span>
                ...
                <div className="nft-create-mint-mode">
                  <div className="nft-create-mint-mode-header">
                    ...
                    <span className="nft-create-tooltip-wrap nft-create-tooltip-details">
                      <span className="nft-create-tooltip-icon" aria-hidden>?</span>
                      <span className="nft-create-tooltip-text">
                        Random mint: minters get a random NFT. Pick & mint: minters select which NFT they want before minting.
                      </span>
                    </span>
                  </div>
                  <div className="nft-create-mint-mode-grid" role="radiogroup" aria-label="Mint mode">
                    {MINT_MODES.map((opt) => {
                      const isSelected = mintMode === opt.value
                      return (
                        <button
                          key={opt.value}
                          type="button"
                          ...
                          className={`nft-create-mint-mode-card ${isSelected ? 'nft-create-mint-mode-card--selected' : ''}`}
                ...
                <div className="nft-create-details-options-panel">
                  ...
                  <div className="nft-create-details-switch-row">
                    <button
                      type="button"
                      role="switch"
                      aria-checked={freezeCollection}
                      className={`nft-create-switch ${freezeCollection ? 'on' : ''}`}
                      onClick={() => setFreezeCollection(!freezeCollection)}
                    >
                      <span className="nft-create-switch-thumb" />
                    </button>
                    <div className="nft-create-details-switch-copy">
                      <div className="nft-create-details-switch-title">
                        <strong>Freeze Collection</strong>
                      </div>
                      <span className="nft-create-details-switch-desc">While frozen, your NFTs cannot be traded. They will become unfreezeable after sellout or after a predetermined unfreeze date.</span>
                    </div>
                  </div>
                  {freezeCollection && (
                    <div className="nft-create-details-freeze-until-row">
                      <div className="nft-create-details-freeze-until-copy">
                        <label className="nft-create-details-label" htmlFor="create-freeze-until-date">
                          Unfreeze at date (optional)
                        </label>
                        <p className="nft-create-details-freeze-until-desc">Trading stays frozen until this date. Leave empty to only unfreeze when sold out. Times are in your local timezone.</p>
                        <FreezeUntilDatePicker
                          id="create-freeze-until-date"
                          value={freezeUntilDate}
                          onChange={setFreezeUntilDate}
                          placeholder="Pick date & time"
                          className="nft-create-freeze-until-input"
                        />
                      </div>
                    </div>
                  )}
                  <div className="nft-create-details-switch-row">
                    <button
                      type="button"
                      role="switch"
                      aria-checked={revealLater}
                      className={`nft-create-switch ${revealLater ? 'on' : ''}`}
                      onClick={() => setRevealLater(!revealLater)}
                    >
                      <span className="nft-create-switch-thumb" />
                    </button>
                    <div className="nft-create-details-switch-copy">
                      ...
                      <span className="nft-create-details-switch-desc">You want to also use placeholder art for your NFT collection to be revealed at a later time.</span>
                    </div>
                  </div>
                  <div className="nft-create-details-switch-row">
                    <button
                      ...
                      aria-disabled
                      disabled
                      className="nft-create-switch on disabled"
                    >
                      <span className="nft-create-switch-thumb" />
                    </button>
                    <div className="nft-create-details-switch-copy">
                      <div className="nft-create-details-switch-title">
                        <strong>Enforce Royalties</strong>
                      </div>
                      <span className="nft-create-details-switch-desc">Enforce royalties.</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {sectionRevenueExpanded && (
              <div className="nft-create-step1-collapse-body">
                  ...
                  <div className="nft-create-display-row">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (Number.isNaN(v)) setRoyaltyPercent(0)
                        else setRoyaltyPercent(Math.min(50, Math.max(0, v)))
                      }}
                ...
                <div className="nft-create-split-panel">
                  ...
                  {royaltySplitError && (
                    <div className="nft-create-split-error-banner" role="alert">
                      {royaltySplitError}
                    </div>
                  )}
                  {Math.abs(royaltyTotal - 100) > 0.01 && (
                    <div className="nft-create-split-remaining" role="status">Remaining: {(100 - royaltyTotal).toFixed(1)}%</div>
                  )}
                      ...
                      return (
                        <li key={i} className="nft-create-split-row-card">
                          <div className="nft-create-split-row-top">
                            ...
                            {royaltyConfig.length > 1 && (
                              <button
                                type="button"
                                className="nft-create-split-remove-btn"
                                onClick={() => removeRoyaltyConfig(i)}
                          ...
                          <div className="nft-create-split-row-fields">
                            <div className="nft-create-split-field nft-create-split-field-share">
                              ...
                              <div className="nft-create-input-adorned nft-create-input-adorned-start">
                                ...
                                <input
                                  id={`royalty-share-${i}`}
                                  type="number"
                                  name={`royaltyConfig.${i}.share`}
                                  min={0}
                                  max={100}
                                  step={1}
                                  className="nft-create-input"
                                  value={row.share}
                                  onChange={(e) => updateRoyaltyConfig(i, 'share', e.target.value)}
                                />
                              </div>
                            </div>
                            <div className="nft-create-split-field nft-create-split-field-address">
                              ...
                              <input
                                id={`royalty-address-${i}`}
                                type="text"
                                name={`royaltyConfig.${i}.address`}
                                className="nft-create-input"
                                value={row.address}
                                onChange={(e) => updateRoyaltyConfig(i, 'address', e.target.value)}
                  ...
                  <button type="button" className="nft-create-split-add-btn" onClick={addRoyaltyConfig} disabled={royaltyConfig.length >= ROYALTY_SPLIT_MAX}>
                    Add recipient
                  </button>
                </div>
                ...
                <div className="nft-create-split-panel">
                  ...
                  {Math.abs(fundReceiverTotal - 100) > 0.01 && (
                    <div className="nft-create-split-remaining" role="status">Remaining: {(100 - fundReceiverTotal).toFixed(1)}%</div>
                  )}
                  <ul className="nft-create-split-list">
                  ...
                  <button type="button" className="nft-create-split-add-btn" onClick={addFundReceiver} disabled={fundReceivers.length >= ROYALTY_SPLIT_MAX}>
                    Add recipient
                  </button>
                </div>
              </div>
