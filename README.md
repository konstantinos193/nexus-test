...
# Milestone 2 – NeXus Frontend: NFT Asset & Metadata Processing

*A modern, dark-themed NFT launchpad that doesn't take itself too seriously (but still works like a charm)*

**Deliverable scope:** Off-chain NFT preparation features including **metadata generation** — the Creator toolkit at **`/tools`** and the **NFT Layer Generator** at **`/tools/nft-asset`** (equivalent to `http://localhost:3000/tools` and `http://localhost:3000/tools/nft-asset`).
...
**Important:** Run the dev server from the **Frontend** directory (`cd Frontend` then `yarn dev`). Running from the repo root can cause `GET / 500` and errors like `Cannot find module 'next/dist/pages/_error'` or `Can't resolve 'next-flight-client-entry-loader'`.

**If you see `GET / 500` or "Module not found" for Next.js internals:** Do a clean reinstall from the Frontend folder:
