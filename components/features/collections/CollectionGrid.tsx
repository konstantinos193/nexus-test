 ...
 * Wireframe spec: 3 columns desktop, 2 tablet, 2 mobile
 * This is the grid that shows all the collections
 * Because lists are boring and grids are where it's at
 *
 * Features:
 * - Responsive grid (2 mobile, 2 tablet, 3 desktop - matching homepage)
...
interface CollectionGridProps {
  ...
  loading?: boolean
  /** Dashboard mode: use DashboardCollectionCard with Edit/Continue */
  mode?: 'default' | 'dashboard'
}

export default function CollectionGrid({ collections, loading, mode = 'default' }: CollectionGridProps) {
  ...
  // Wireframe: 3 cols desktop, 2 tablet, 2 mobile (matching homepage)
  const CardComponent = mode === 'dashboard' ? DashboardCollectionCard : CollectionCard
  return (
    <div className={styles.grid}>
      {collections.map((collection) => (
        <CardComponent key={collection.id} collection={collection} />
      ))}
    </div>
  )
}
