...
async function main() {
  ...
  if (argv.length < 3) {
    console.error('Usage: node scripts/simulate-create-collection.js <rpcUrl> <programId> <ixDataHex> [authorityKeypair.json]');
    process.exit(1);
  }
  ...
  // For simulation, use authority as mint pubkey (match frontend placeholder behavior)
  ...
  const platformWallet = authority.publicKey; // replace if you know platform wallet
  ...
  const keys = [
    ...
    { pubkey: authority.publicKey,     isSigner: false, isWritable: true  }, // creator_wallet
  ...
  // sign with authority so simulation has signatures
  tx.sign(authority);
