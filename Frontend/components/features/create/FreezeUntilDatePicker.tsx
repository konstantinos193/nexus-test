...
/**
 * FreezeUntilDatePicker - Date-time picker for "Unfreeze at date".
 * Opens a calendar popover in the user's local timezone.
 * Uses a portal to render at body level (avoids parent overflow clipping).
 * Value stored as datetime-local string (YYYY-MM-DDTHH:mm).
 *
 * @author Juan - The developer who built a calendar from scratch
          ...
          >
            <div className="nft-create-freeze-until-calendar">
              <div className="nft-create-freeze-until-calendar-header">
                <button type="button" className="nft-create-freeze-until-nav" onClick={() => setViewDate((d) => new Date(d.getFullYear(), d.getMonth() - 1, 1))} aria-label="Previous month">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
                </button>
                <span className="nft-create-freeze-until-month-label">{monthLabel}</span>
                <button type="button" className="nft-create-freeze-until-nav" onClick={() => setViewDate((d) => new Date(d.getFullYear(), d.getMonth() + 1, 1))} aria-label="Next month">
            ...
            <div className="nft-create-freeze-until-time">
              ...
              <div className="nft-create-freeze-until-time-inputs">
                <div className="nft-create-freeze-until-time-field">
                  ...
                  <input id={`${id}-hour`} type="number" min={0} max={23} value={hour} onChange={(e) => handleTimeChange(Number(e.target.value) || 0, minute)} onBlur={(e) => handleTimeChange(Math.min(23, Math.max(0, Number(e.target.value) || 0)), minute)} className="nft-create-freeze-until-time-input" />
                </div>
                ...
                <div className="nft-create-freeze-until-time-field">
                  ...
                  <input id={`${id}-minute`} type="number" min={0} max={59} value={String(minute).padStart(2, '0')} onChange={(e) => handleTimeChange(hour, Number(e.target.value) || 0)} onBlur={(e) => handleTimeChange(hour, Math.min(59, Math.max(0, Number(e.target.value) || 0)))} className="nft-create-freeze-until-time-input" />
                </div>
              </div>
            </div>
...
// Coded by Juan - a calendar, from scratch, in React. We're brave.
