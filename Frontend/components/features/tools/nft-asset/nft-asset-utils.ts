...
export function groupByFolder(files: File[]): Map<string, File[]> {
  ...
  for (const f of files) {
    ...
    pathMap.set(f.name, parts)
  }

  // Determine the grouping level
  const allParts = Array.from(pathMap.values())
  if (allParts.length === 0) return map

  const firstSegments = allParts.map((s) => s[0])
  const allSameFirst =
    firstSegments.length > 0 &&
    firstSegments.every((s) => s === firstSegments[0]) &&
    allParts.every((s) => s.length >= 2)
  const groupByIndex = allSameFirst ? 1 : 0

  // Group files
  for (const f of files) {
    const parts = pathMap.get(f.name) || []
